...
How do the PIDA projects compare to a cost-equivalent package of links selected purely on partial-equilibrium economic criteria? To answer this, I rank all candidate links (existing edges plus proposed new links) by their consensus \$/min/\$ ratio across the three friction scenarios analysed in \S\ref{sec:UUEL}, and accumulate the highest-yielding links until the total cost reaches \$10.1B. This greedy ``top-N consensus'' benchmark contains 351 links (29,600km of upgrades $+$ 4,200km of new construction; \$10.08B) and yields a joint MA gain of \textbf{18.4\%} (frictionless; 30.5\,\$/min/\$) and \textbf{17.1\%} (frictions; 13.3\,\$/min/\$)---more than double the frictionless PIDA gain and roughly nine times the under-frictions PIDA gain on the same budget. As Figure \ref{fig:PIDA_vs_Top10B} illustrates, the two packages overlap on only 31 links (purple), while PIDA's other 163 links (orange) include many remote cross-border segments that the PE-optimal benchmark would not prioritise; conversely, the optimal package would invest in 320 additional high-yield links (blue) in populated regions, crop belts, and metropolitan corridors that PIDA does not target. The cost-effectiveness gap is consistent with an analogous evaluation for the CEMAC region (companion work by the author), where planned regional projects yield 28.6\% MA at 2.14 \$/min/\$ versus 44\% at 2.8 \$/min/\$ for the cost-matched PE-optimal package. \newline

\begin{figure}[H] \vspace{-2mm}
\caption{\label{fig:PIDA_vs_Top10B} PIDA Projects vs. Cost-Matched Top-N Consensus Package ($\approx$\$10B)}
\vspace{2mm}
\centering
\pidaIG{width=0.7\textwidth}{../figures/transport_network/PIDA/trans_africa_network_PIDA_vs_top10B_consensus.pdf}
\raggedright
\scriptsize
\emph{Notes:} Purple: links included in both PIDA and the top-N consensus benchmark. Orange: links in PIDA only. Blue: links in the top-N consensus benchmark only. The benchmark contains the highest-yielding links (sorted by consensus \$/min/\$ across the three friction scenarios) until the cumulative cost matches the \$10.1B PIDA total. \vspace{-1mm}
\end{figure}
...
To complement the PE assessment, I run the welfare-maximising planner of \citet{fajgelbaum2020optimal} with the dimension-reduction strategy of \S4 (22 differentiated goods produced by selected cities; $\sigma = 3.8$ Armington elasticity) at a budget of $K_i = \$10.1$B 2015 USD---exactly the PIDA package cost. I compute the optimal allocation for four planner variants: standard ($\gamma = \beta = 1$) and increasing-returns to infrastructure (IRS, $\gamma^\ast = \beta^2/\gamma$), each with utilitarian ($\rho = 0$) and inequality-averse ($\rho = 2$) preferences. Frictions are intentionally omitted in all four runs: as established in \S4 of this paper, border frictions have only minor effects on the \textit{shape} of the GE-optimal allocation (they primarily scale levels), so adding them would not change the spatial comparison with PIDA in any meaningful way. For the same reason, the OSBP component---whose effect is precisely identified in the PE analysis above---is left aside in the GE counterfactual. \newline

Figure \ref{fig:PIDA_GE_main} overlays the PIDA package (navy dashed) on each planner's optimal allocation (yellow-to-red intensity). Two structural patterns emerge. \textit{First}, the standard planner---facing strong congestion at the calibrated $\gamma = \beta = 1$---spends a majority of the budget on \textit{new} construction (\$6.3B / 10.6K\,km new vs \$3.8B / 8.6K\,km upgrades under utilitarian preferences; \$3.5B / 6.1K\,km new vs \$6.6B / 15.1K\,km upgrades under inequality aversion), routing trade along the periphery of the existing network. \textit{Second}, the IRS planner---facing weak congestion---spends virtually the entire budget on upgrading existing links (24.8--25.0K\,km, essentially \$0 on new construction), reinforcing the highest-density corridors. This dichotomy mirrors the continent-wide GE findings of \S4 (``Optimal Trans-African Network Investments''): when congestion is strong, optimal road investments diversify routes; when congestion is weak, they concentrate on densifying the trunk network. \newline

\begin{figure}[H] \vspace{-2mm}
\caption{\label{fig:PIDA_GE_main} Welfare-Maximising GE Investments at \$10.1B vs PIDA Overlay}
\vspace{2mm}
\resizebox{\textwidth}{!}{
\begin{tabular}{@{}c@{}c@{}}
\textbf{Standard planner ($\rho = 0$)} & \textbf{Standard planner ($\rho = 2$, IA)} \\
WG: 0.09\%  |  CG: 0.10\%  |  MA: 5.2\%  |  PIDA-share: 2.1\% &
WG: 0.08\%  |  CG: 0.09\%  |  MA: 5.4\%  |  PIDA-share: 3.7\% \\
\pidaIG{width=0.48\textwidth}{../figures/transport_network/PIDA/GE/trans_africa_network_GE_add_22g_10b_fixed_sigma3.8_rho0_julia_perc_ug_with_PIDA.pdf} &
\pidaIG{width=0.48\textwidth}{../figures/transport_network/PIDA/GE/trans_africa_network_GE_add_22g_10b_fixed_sigma3.8_rho2_julia_perc_ug_with_PIDA.pdf} \\
\textbf{IRS planner ($\rho = 0$)} & \textbf{IRS planner ($\rho = 2$, IA)} \\
WG: 0.21\%  |  CG: 0.24\%  |  MA: 6.3\%  |  PIDA-share: 4.3\% &
WG: 0.18\%  |  CG: 0.24\%  |  MA: 5.6\%  |  PIDA-share: 5.7\% \\
\pidaIG{width=0.48\textwidth}{../figures/transport_network/PIDA/GE/trans_africa_network_GE_add_22g_10b_fixed_irs_na_sigma3.8_rho0_julia_perc_ug_with_PIDA.pdf} &
\pidaIG{width=0.48\textwidth}{../figures/transport_network/PIDA/GE/trans_africa_network_GE_add_22g_10b_fixed_irs_na_sigma3.8_rho2_julia_perc_ug_with_PIDA.pdf} \\
\end{tabular}
}
\raggedright
\scriptsize
\emph{Notes:} Each panel shows the welfare-maximising allocation of a \$10.1B infrastructure budget (PIDA-equivalent) under different planner preferences. Yellow-to-red links indicate the fractional upgrade ($\Delta\%$ UG) chosen by the planner; navy dashed lines overlay the PIDA package for visual comparison. WG = aggregate welfare gain, CG = consumption gain, MA = market-access gain; PIDA-share is the fraction of the optimal budget falling on links also flagged as PIDA. Frictions are omitted (their effect on the allocation shape is shown to be minor in \S4). \vspace{-1mm}
\end{figure}

The most striking finding from the spatial overlap is that the GE-optimal planner allocates only \textbf{2.1\%--5.7\%} of its budget to PIDA-flagged links, depending on preferences---i.e., \textit{at most one dollar in twenty} of an optimally-spent \$10.1B would have gone to a PIDA road. The overlap rises modestly under inequality aversion (the planner places somewhat more weight on remote regions PIDA targets, such as the Sahel and Central African corridors) and under IRS preferences (the planner concentrates on existing trunk roads, some of which PIDA also targets), but never exceeds 6\%. The remaining 94--98\% of the budget is invested in trans-Saharan east-west links, the Maghreb--Sahel arteries, the Trans-East-African corridor north of Lake Victoria, and metropolitan upgrades around Lagos, Kinshasa, Khartoum, and Johannesburg---none of which feature prominently in PIDA. \newline

In welfare terms, the GE-optimal allocation at \$10.1B yields between \textbf{0.08\% and 0.21\%} aggregate welfare gain. The IRS variants generate the largest welfare gains (0.18--0.21\%, more than twice the standard-planner figures), consistent with the broader finding that weak inter-city congestion---and hence concentration of investment on upgrading the existing network---is the welfare-maximising regime in the African context. The corresponding MA gains range from 5.2\% (standard, $\rho=0$) to 6.3\% (IRS, $\rho=0$); note that these are GE-equilibrium MA values computed on the welfare-maximising network, not the directly-comparable PE Top-N MA figures from \S\ref{sec:PIDAeval} above (which max \textit{MA} given a budget rather than welfare). \newline

Putting PE and GE together, the analysis suggests that the PIDA pipeline---while delivering meaningful PE returns (8.5\% frictionless MA gain, additional 3.6pp from OSBPs)---is spatially mis-aligned with welfare-maximising priorities at the continental scale. A planner with the same \$10.1B budget would invest in only a small fraction of PIDA roads and would instead concentrate on trunk-upgrading and selective new links in West and East Africa. Welfare gains achievable from such a re-allocated package (0.08\%--0.21\%) are modest in absolute terms, reflecting both the small size of the budget relative to the \$162B total cost of all proposed continental works and the very dispersed nature of welfare benefits in a multi-good Armington economy. Nonetheless, the qualitative conclusion is consistent across all four planner variants: PIDA's spatial allocation captures at most one-in-twenty dollars of what an optimal planner would have spent. \newline
