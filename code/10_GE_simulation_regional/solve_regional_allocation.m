...
res_edges = readtable(sprintf('results/transport_network/GE/regional/edges_results_%s.csv', filename));
infra_matrix_opt = vec_to_res(n, res_edges.Ijk, edges);

% Check: should be >= 1
tmp = res_edges.Ijk ./ res_to_vec(infra_matrix, edges);
[min(tmp), max(tmp)]

% Solve allocation from optimal infrastructure investments
strcat("Started P_stat on ", datestr(datetime('now')))
res_opt = solve_allocation(param, graph, infra_matrix_opt, true);
...
writetable(res_nodes, sprintf('results/transport_network/GE/regional/nodes_results_%s_%s.csv', filename, fileext))
...
writetable(res_edges, sprintf('results/transport_network/GE/regional/edges_results_%s_%s.csv', filename, fileext));
